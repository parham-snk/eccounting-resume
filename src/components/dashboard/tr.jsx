...
const Tr = (props) => {
    let { invoice_id, doc_id, custom_date, invoice_type, total_price, eccount_name, index } = props.data
    // let d=new Date(date).toLocaleString("fa-IR")
    // let dd =d.split(",")[0]

    return (
        <tr className="py-2 border-b-0 hover:bg-gray-200 dark:hover:bg-zinc-900 dark:hover:text-white cursor-pointer" onDoubleClick={() => {
            props.setInvoiceItem(props.data)
        }}>
            <td className="py-4">{index>=0&&index+1}</td>
            <td className="py-4">{doc_id ? doc_id : null}</td>
